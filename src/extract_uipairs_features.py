...
def get_other_basic_ui_features(tmp_features):
    ...
    other_features['buy_per_buyanything'] = float(tmp_features['buy_counts'])/(user_features[user_id]['buy_counts'] + 1)   #累计购买/(用户)总购买次数
    other_features['behaviordays_per_activedays'] = float(tmp_features['buy_counts'])/(user_features[user_id]['active_days_count'] + 1)   #累计访问天数/(用户)活跃天数
    other_features['buydays_per_buyanythingdays'] = float(tmp_features['buy_counts'])/(user_features[user_id]['buy_days_count'] + 1)   #累计购买天数/(用户)购买天数
