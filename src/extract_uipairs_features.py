...
def initial_uipair_features(uipair_features):

    ###基础属性
    for basic_features in basic_feature_behaviour.values():
        uipair_features[basic_features[0]] = 0    #xxx_times # 销量 # 购物车量 # 收藏量 # 点击量
        uipair_features[basic_features[1]] = {}   #xxx_user {user，count} # 购物车用户 # 消费用户
        uipair_features[basic_features[2]] = {}   #user_first_xxx_time


    uipair_features['user'] = []
    uipair_features['eachday_behavior_counts']= {}  # 每天行为总数 {time:[0,1,2,3]}++++++++++

    ###扩展属性
    uipair_features['lastday_behavior_counts'] = []  # 最后一天行为次数++++++++
    uipair_features['total_user'] = 0.0   # 总用户数
    uipair_features['total_cart_people'] = 0.0    # 总购物车人数
    uipair_features['total_buy_people'] = 0.0     # 总购买人数
    uipair_features['total_favor_people'] = 0.0   # 总收藏人数
    uipair_features['total_click_people'] = 0.0   # 总收藏人数
    uipair_features['multiple_buy_user'] = 0.0    # 多次购买的用户数
    uipair_features['once_click_user'] = 0.0  # 只点击过一次的用户数
    uipair_features['careful_user'] = 0.0 # 不在初次访问品牌时购买的订单数 ???????
...
def extract_items_features(train_file_path):
    ...
    item_features = {}
    item_features = initial_uipair_features(item_features)
    # 输出栏位名
    ...
    for line in train_file:
        ...
        if not item_id == pre_item_id:
            ...
            item_features = initial_uipair_features(item_features)  # 初始化置空item_features
